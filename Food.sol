...
contract Food is ERC1155, Ownable {
	...
	uint256 public foodSold;
	mapping(uint256 => uint256) private _totalSupply;
	mapping(uint256 => string) public _tokenURIs;
    
    string public name = "Food";
	...
	function buyFood(uint256 _howMany) public payable {
	    require(foodSold + _howMany <= 9949, "All sold out");
		require(itemPrice.mul(_howMany) == msg.value, "insufficient ETH");
		uint256 id = foodSold;
		for (uint256 i = 0; i < _howMany; i++) {
			getFood(id+i);
		}
        foodSold = foodSold + _howMany;
	}

	function getFood(uint256 id) private {
